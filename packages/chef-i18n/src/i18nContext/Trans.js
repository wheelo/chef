  ...
  children.forEach((child, i) => {
    ...
    if (typeof child === 'string') {
      ...
      if (format && keys.length === 1) {
        ...
        warn(
          `chef-i18n: the passed in object contained more than one variable - the object should look like {{ value, format }} where format is optional.`,
          child
        );
      }
    } else {
...
export class TransComponent extends React.Component {
  // TODO: 目前只有children / i18n / data这三个字段有用
  render() {
    const {
      children,
      count,
    ...
    const key = id || (hashTransKey ? hashTransKey(defaultValue) : defaultValue);
    ...
    if (key && data) {
      // interpolate
      translation = translation.replace(/\${(.*?)}/g, (_, k) => data[k])
    }
    ...
    if (reactI18nextOptions.exposeNamespace) {
      ...
      if (
        ...
        id.indexOf(i18n.options.nsSeparator) > -1
      ) {
        const parts = id.split(i18n.options.nsSeparator);
        ns = parts[0];
      }
