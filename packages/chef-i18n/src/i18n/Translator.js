...
class Translator extends EventEmitter {
  ...
  resolve(keys, options = {}) {
    ...
    keys.forEach((k) => {
      ...
      codes.forEach(code => {
        ...
        // TODO: 这边key不处理plural的场景
        // iterate over finalKeys starting with most specific pluralkey (-> contextkey only) -> singularkey only
        let possibleKey;
        while (possibleKey = finalKeys.pop()) {
          if (!this.isValidLookup(found)) {
            found = this.getResource(code, possibleKey, options);
          }
        }
      });
    });

    return { res: found, usedKey, usedLng };
  }
  ...
  getResource(code, key, options = {}) {
    // 真正调用的逻辑
    return this.resourceStore.getLan(code, key, options);
  }
}
